# AstrBot 部署教程：宝塔面板 + NapCat 搭建 QQ 机器人

> 作者：Yuimi-chaya。来源：[原文](https://github.com/Yuimi-chaya/Yuimi-chaya.github.io/blob/4fd072b8efb10e34403037d6fda2f068d1acd934/src/content/blog/astrbot-napcat-baota-deploy.md)。
>
> 本地阅读副本：保留正文；移除站点 frontmatter 和封面引用，补回标题，转换站点专用代码块、图文容器与站内链接，图片改为仓库内相对路径。旧文中的公网端口、空 token、latest 镜像及模型名称是当时的示例，不作为本书默认配置。开始操作前请先读[本书部署章](../chapters/07-astrbot-deployment.md)。
>
> 原创文字依原文 CC BY-NC-SA 4.0 许可；代码与图片范围见[来源与许可](../LICENSE.md)。
...
![容器编排中开放 AstrBot 端口](../assets/blog-assets/astrbot-napcat-baota/08-compose-ports.webp)
...
![NapCat token 日志](../assets/blog-assets/astrbot-napcat-baota/13-napcat-token-log.webp)
...
![NapCat 登录页](../assets/blog-assets/astrbot-napcat-baota/14-napcat-login.webp)
...
![AstrBot 配置 AI 模型入口](../assets/blog-assets/astrbot-napcat-baota/16-astrbot-model-entry.webp)

新增提供商时可以选择 DeepSeek,也可以选择 OpenAI Compatible 后填写兼容地址

![AstrBot 选择模型提供商](../assets/blog-assets/astrbot-napcat-baota/17-astrbot-provider-select.webp)
...
![DeepSeek 创建 API Key](../assets/blog-assets/astrbot-napcat-baota/18-deepseek-create-key.webp)

创建后立刻复制并保存 API Key.关闭弹窗后,通常无法再次查看完整 key

![DeepSeek 复制 API Key](../assets/blog-assets/astrbot-napcat-baota/19-deepseek-copy-key.webp)

回到 AstrBot,在模型提供方里填入刚才复制的 API Key,确认 API Base URL 正确

![AstrBot 填入 API Key](../assets/blog-assets/astrbot-napcat-baota/20-astrbot-api-key.webp)

点击“获取模型列表”,把需要使用的模型添加到已配置模型中

![AstrBot 获取模型列表](../assets/blog-assets/astrbot-napcat-baota/21-astrbot-model-list.webp)
...
![AstrBot 创建 OneBot v11 机器人](../assets/blog-assets/astrbot-napcat-baota/24-create-onebot-robot.webp)

### 在 NapCat 中新增 WebSocket 客户端

进入 NapCat WebUI 的“网络配置”,新增一个 WebSocket 客户端

![NapCat 新增 WebSocket 客户端](../assets/blog-assets/astrbot-napcat-baota/25-create-napcat-ws-client.webp)
