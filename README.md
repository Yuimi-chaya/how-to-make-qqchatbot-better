...
读这本书时，可以先问每项能力想改善哪一种相处感，再去看它由谁实现。输入状态的意义，是让模型生成期间的等待有一个可见信号，减少“突然断线”的感觉；防抖和撤回，是让用户还有机会把没说完的话说完；纯表情回复，是让没有明确任务的随手互动不必被翻译成一篇说明。它们解决的是不同的体验问题，不能只因为都被称为“拟人化”就装在一起。
