...
等模型生成时，QQ 如果能显示“对方正在输入…”，等待会有一个可见信号。它的意义不在于伪造模型脑中的某种状态，而在于告诉用户：这次消息还在处理，不是对话突然断了。对于需要等待模型、工具或图片处理的私聊，这个信号可以让延迟更容易被接受；如果状态一直挂着、旧回复已经排队却还显示正在输入，反而会放大不真实感。因此它应被看成等待体验的补丁，而不是“真人感”的总开关。

[input_state_by_nc](https://github.com/ctrlkk/astrbot_plugin_input_state_by_nc) 虽以 NapCat 命名，代码实际判断的是 AstrBot 的 OneBot 私聊事件，再向所接协议端调用 `set_input_status`；在 LLM 请求期间持续上报，回复处理前还会补报一次。它不让模型更快，也不会合并用户补发的消息或取消已排队的旧回复；显示了“正在输入”，不表示机器人能像人一样边看新消息边改正在生成的答案。
...
它值得尝试的原因，是它处理了“人类输入是连续的，模型请求却按单次事件启动”这个错位。用户分几条消息把一句话说完时，先短暂收集可以减少模型对半句话做出完整回答；用户在等待期间改口时，让旧回复失效可以避免聊天窗口继续播放已经不适用的内容。这里改善的是回合边界和发送后果，不是让模型突然获得更强的理解能力。
