...
先从一个常见误会开始：机器人“不好聊”，不一定是模型不会说话。它可能根本没收到你的补充，可能已经生成了合适的内容却被宿主拆成了奇怪的气泡，也可能协议端没有把图片、撤回或主动消息这类动作送到 QQ。要改善体验，先知道一条消息怎样从 QQ 走到模型，再怎样回来。

下面这张图不是组件清单，而是一段私聊为什么会变成现在这个样子的路线图：
...
这张图的用处不是让你背下组件名称，而是帮助你找到“哪里改变了体验”。例如，模型原文很合适，QQ 却把它拖成一串气泡，问题多半发生在输出处理；模型已经生成了回复，但等待期间没有任何可见状态，输入状态插件解决的是等待的感受，不是模型的理解能力；用户连续补发三条消息却被分别回答，应该先看宿主怎样组织输入，而不是先责怪角色卡。后面的每项配置都应回到这条链路上解释。
...
“反向”不是消息只能单向传输，而是连接建立的方向。典型目标地址是 `ws://astrbot:6199/ws`，这里的 `astrbot` 只适用于两个容器处在同一用户自定义 Docker 网络、服务名可解析的情况。同一网络也可以用 AstrBot 容器的内部 IP 连接，但容器重建可能换 IP，长期配置优先用服务名。

如果两者不在一个网络里，这个名字就不一定可用；如果你在协议端容器里填 `127.0.0.1`，通常指向它自己，而不是 AstrBot。第 7 章用 NapCat 给出同一 Compose 的例子；[LLBot 官方脚本安装实例](../guides/llbot-with-astrbot.md)说明两个独立项目怎样加入共享网络。
